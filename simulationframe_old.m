function [imu,gnss] = simulationframe(imu, gnss, carsim_data, time, ab_dyn, wb_dyn)
    ...
    imu.ar = [carsim_data.asx.data(time), carsim_data.asy.data(time), carsim_data.asz.data(time)]'+ab_dyn+0.01*(rand(3,1)-0.5);
    imu.wr = [carsim_data.wsx.data(time), carsim_data.wsy.data(time), carsim_data.wsz.data(time)]'+wb_dyn+0.001*(rand(3,1)-0.5);
